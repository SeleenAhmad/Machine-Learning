...
plt.ylabel("Importance")
plt.show()
l_coefs=l_model.coef_
lin_df= pd.DataFrame({"Feature":features,"Coefficients":l_coefs}).sort_values(by="Coefficients",key=abs, ascending= False)
plt.figure(figsize=(8,4))
plt.bar(lin_df["Feature"],lin_df["Coefficients"],color="lightgreen")
plt.title("Linear Regression Feature importance via Coefficients")
